...
\.
COPY public.actor (actor_id, first_name, last_name, last_update) FROM 'dvdrental/3057.dat';
...
\.
COPY public.address (address_id, address, address2, district, city_id, postal_code, phone, last_update) FROM 'dvdrental/3065.dat';
...
\.
COPY public.category (category_id, name, last_update) FROM 'dvdrental/3059.dat';
...
\.
COPY public.city (city_id, city, country_id, last_update) FROM 'dvdrental/3067.dat';
...
\.
COPY public.country (country_id, country, last_update) FROM 'dvdrental/3069.dat';
...
\.
COPY public.customer (customer_id, store_id, first_name, last_name, email, address_id, activebool, create_date, last_update, active) FROM 'dvdrental/3055.dat';
...
\.
COPY public.film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length, replacement_cost, rating, last_update, special_features, fulltext) FROM 'dvdrental/3061.dat';
...
\.
COPY public.film_actor (actor_id, film_id, last_update) FROM 'dvdrental/3062.dat';
...
\.
COPY public.film_category (film_id, category_id, last_update) FROM 'dvdrental/3063.dat';
...
\.
COPY public.inventory (inventory_id, film_id, store_id, last_update) FROM 'dvdrental/3071.dat';
...
\.
COPY public.language (language_id, name, last_update) FROM 'dvdrental/3073.dat';
...
\.
COPY public.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date) FROM 'dvdrental/3075.dat';
...
\.
COPY public.rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update) FROM 'dvdrental/3077.dat';
...
\.
COPY public.staff (staff_id, first_name, last_name, address_id, email, store_id, active, username, password, last_update, picture) FROM 'dvdrental/3079.dat';
...
\.
COPY public.store (store_id, manager_staff_id, address_id, last_update) FROM 'dvdrental/3081.dat';
...
-- 1) Query to find the customer that had the most rentals and retrieve their first name and last name as a single column, along with the count of rentals per customer in descending order.
...
--2) Query to find out if the customer that had the most rentals rented any movie more than once.
SELECT CONCAT(c.first_name, ' ', c.last_name) AS customer_name, r.rental_id, r.inventory_id, f.title, COUNT(*) AS rental_count
FROM customer AS c
...
GROUP BY c.first_name, c.last_name, r.rental_id, r.inventory_id, f.title
HAVING COUNT(*) > 1;


SELECT CONCAT(c.first_name, ' ', c.last_name) AS customer_name, f.title, COUNT(*) AS rental_count
FROM customer AS c
JOIN rental AS r ON c.customer_id = r.customer_id
JOIN inventory AS i ON r.inventory_id = i.inventory_id
JOIN film AS f ON i.film_id = f.film_id
...
-- 3) Query to find the name of the customer’s favorite movie category and the total number of films they rented in that category.
SELECT CONCAT(c.first_name, ' ', c.last_name) AS customer_name, cat.name AS favorite_category, COUNT(*) AS rental_count
FROM customer AS c
JOIN rental AS r ON c.customer_id = r.customer_id
JOIN inventory AS i ON r.inventory_id = i.inventory_id
JOIN film AS f ON i.film_id = f.film_id
...
-- 4) Trigger to delete a customer if they become inactive.
CREATE OR REPLACE FUNCTION delete_inactive_customer()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_delete_inactive_customer
AFTER UPDATE ON customer
FOR EACH ROW
EXECUTE FUNCTION delete_inactive_customer();
